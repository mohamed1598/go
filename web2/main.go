...
func write(writter http.ResponseWriter, msg string) {
	_, err := writter.Write([]byte(msg))
	if err != nil {
		log.Fatal(err)
	}
}
...
func divideHandler(writer http.ResponseWriter, requiest *http.Request) {
	v, err := getQuotient(5, 4)
	if err != nil {
		write(writer, "can't divide by zero\n")
	} else {
		write(writer, fmt.Sprintf("5 /4 = %.2f \n", v))
	}
}
